...
function getProjectTypeFromSite(site: string): string {
  const siteToProjectType: Record<string, string> = {
    'rr-san-antonio-tx-1031-exchange': '1031 Exchange',
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.1031exchangeofsanantonio.com'
    ...
    const payload = {
      ...body,
      projectType: getProjectTypeFromSite('rr-san-antonio-tx-1031-exchange'),
      contractorEmail: process.env.CONTRACTOR_EMAIL || '',
      timestamp: new Date().toISOString(),
      source: process.env.NEXT_PUBLIC_SOURCE || '1031 Exchange of San Antonio Website',
      submitted_at: new Date().toISOString(),
      website_url: siteUrl,
      // helpful tracing for Zapier
      _meta: {
        site: 'rr-san-antonio-tx-1031-exchange',
        route: '/api/contact',
      },
    }
